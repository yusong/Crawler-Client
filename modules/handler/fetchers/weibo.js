define(['modules/http_agent'], function(http_agent){
	...
	/**
	 * Sina Single Sign On Encoder Module
	 */
	 ...
	 * @param url null
	 */
	Weibo.prototype.login = function(url, callback) {

		/* Login Options */
		var URLS = [
			{
				url : 'http://login.sina.com.cn/sso/prelogin.php?entry=weibo&callback=sinaSSOController.preloginCallBack&su=&rsakt=mod&client=ssologin.js(v1.4.11)',
				option : {
					type: 'GET',
					data: { _ : (new Data()).valueOf() }
				},
			...
			},
			{
				...
				option : {
					type : 'POST',
					headers : { Referer : 'http://weibo.com/?from=bp&lang=zh-cn' }
				}
			},
			{
				option : {
					type : 'GET',
					headers: { Referer : 'http://weibo.com/ajaxlogin.php?framelogin=1&callback=parent.sinaSSOController.feedBackUrlCallBack&sudaref=weibo.com', 'Accept-Charset' : 'utf-8' }
				}
			}			
		];

		/* Door 1 */
		http_agent( URLS[0].url, URLS[0].option, function(err, data){
			if(err) { callback(err); return; }

			data.match(/"servertime":(\d+),/);
			servertime = RegExp.$1;
			data.match(/"nonce":"([\w\d]+)"/);
			nonce = RegExp.$1;
			data.match(/"rsakv":"(\d+)"/);
			rsakv = RegExp.$1;

			var sina = new sinaSSOEncoder.RSAKey();
			sina.setPublic(URLS[0].pubkey, '10001');
			var sp = sina.encrypt([servertime, nonce].join("\t") + "\n" + URLS[0].password);

			/* Door 2 */
			http_agent( URLS[1].url, 
						{ 
							type: URLS[1].option.type, 
						  	headers: URLS[1].option.headers, 
						  	data: {
						  		entry:'weibo',
								gateway:1,
								from:'',
				...
				function(err, data){
					...
					data = data.replace(/[\n\r\f\t\v]/g, '');
					data.match(/location.replace\("(.*)"\)/);
					var targetUrl = RegExp.$1;

					/* Door 3 */
					http_agent( targetUrl, URLS[2].option, function(err, data){
						if(err) callback(err);
						else {
	...
	/**/
	var login_time_span = 25;

	Weibo.prototype.home = function(url, callback) {

		var curDate = new Date().getHours();

		// agent('http://weibo.com/u/2813262187?wvr=5&wvr=5&lf=reg', {type: 'GET', headers: { Referer : 'http://weibo.com/ajaxlogin.php?framelogin=1&callback=parent.sinaSSOController.feedBackUrlCallBack&sudaref=weibo.com', 'Accept-Charset' : 'utf-8' }},
		// 	function(err, data){
		// 		alert(data);
		...
		// 	if(err) {
		// 		console.log('[Fetcher] weibo.home Error');
		// 		callback(err);
		// 	} 
		// 	else {
		// 		console.log(dom);
		...
		console.log('Weibo.prototype.home');
		callback(null, null);
	};
